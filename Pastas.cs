...
class Pastas {
	...
	//Pega o Diretório da Pasta de Arquivos Recents
	private DirectoryInfo recents { get; set; } = new DirectoryInfo(Environment.GetEnvironmentVariable("USERPROFILE") + @"\AppData\Roaming\Microsoft\Windows\Recent\");
	...
	//Retorn o Tamanho (KB, MB ou GB) total da Pasta de Arquivos Recentes
	public string totalRecent(){
		return analisaUnidades(getSizes(contaArquivos(recents)));
	}
	...
	public string mostraTamanhoTotal(){
		return    " ------------------------------------------------"   + "\n" +
				  ...
				  "| Tamanho da Pasta Recents: "  + totalRecent()  + "\n" +
				  " -------------------------------------------------";
	}
